...
!(function(e) {
    ...
    (U.dateRangePickerLanguages = {
        ...
        pt: {
            ...
            "less-than": "O período selecionado não deve ser maior que %d días",
            "more-than": "O período selecionado não deve ser menor que %d días",
            "default-more": "Selecione um período superior a %d días",
            "default-single": "Selecione uma data",
            "default-less": "Selecione um período inferior a %d días",
            "default-range": "Selecione um período de %d a %d días",
    ...
    (U.fn.dateRangePicker = function(f) {
            ...
            if (!e.hasClass("invalid")) {
                ...
                if (
                    (e.addClass("checked"),
                        ...
                        f.time.enabled &&
                        (f.start && h("start", f.start), f.end && h("end", f.end)), !f.singleDate && f.start && f.end && f.start > f.end)
                ) {
                    var a = f.end;
                    (f.end = v(f.start)),
                    (f.start = p(a)),
        ...
        function S(e, t) {
            var a;
            var oDateUtils = new SDateUtils();
            m.find(".start-day").html("..."),
                m.find(".end-day").html("..."),
                m.find(".selected-days").hide(),
                f.start && m.find(".start-day").html(oDateUtils.formatDate(W(new Date(parseInt(f.start))), 'ddd DD-MMM-YYYY')),
                f.end && m.find(".end-day").html(oDateUtils.formatDate(W(new Date(parseInt(f.end))), 'ddd DD-MMM-YYYY')),
        ...
        function j(e, t, a) {
            ...
            if (
                (f.startDate && H(e, f.startDate) < 0 && (r = !1),
                    f.endDate && 0 < H(t, f.endDate) && (r = !1), !r)
            )
                return (
            ...
            m.find("." + t + " .month-name").html(a + " " + n),
                m.find("." + t + " tbody").html(
                    (function(e) {
                        ...
                        for (
                            var l = [], u = 0; u < 6 && "nextMonth" != t[7 * u].type; u++
                        ) {
                            l.push("<tr>");
                            for (var i = 0; i < 7; i++) {
        ...
        function q(e) {
            return (
                (e = Q(e)), !(!f.startDate || !e.endOf("month").isBefore(f.startDate)) ||
